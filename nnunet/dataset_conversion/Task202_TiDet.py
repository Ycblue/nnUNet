...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=int, help='Task number.')
    parser.add_argument('--res', type=int, default=512, help='Image Resolution.')
    parser.add_argument('--microns', type=int, help='Image Microns.')
    parser.add_argument('--source_dir', type=str, help='Image source directory.')
    ...
    if args.source_dir is None: 
        base = '/home/ylan/Project_Human_Bios/dataset/tiles/{}_{}uM_david_split'.format(resolution, microns)
    else: base = args.source_dir
    # this folder should have the training and testing subfolders

    # now start the conversion to nnU-Net:
    # task_name = 'Task120_MassRoadsSeg'
    # task_name = 'Task202_TiDet512_1000uM'
    task_name = 'Task{}_{}_{}uM_david_split'.format(task, resolution, microns)
    # task_name = 'Task202_TiDet'
    ...
    # labels
    labels_dir_tr = join(base, 'training', 'labels')
    ...
    for t in training_cases:
        ...
        convert_2d_image_to_nifti(input_image_file, output_image_file, is_seg=False, spacing=(1,1,1))
        ...
        convert_2d_image_to_nifti(input_segmentation_file, output_seg_file, is_seg=True, transform=lambda x: (x > 250).astype(int)) #, transform=lambda x: (x == 255).astype(int)

    # now do the same for the test set
    labels_dir_ts = join(base, 'testing', 'labels')
    images_dir_ts = join(base, 'testing', 'images')
    ...
    for ts in testing_cases:
        ...
        convert_2d_image_to_nifti(input_segmentation_file, output_seg_file, is_seg=True, spacing=(1,1,1), transform=lambda x: (x > 250).astype(int)) #, transform=lambda x: (x == 255).astype(int)
                                  

    # finally we can call the utility for generating a dataset.json
    generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('Red', 'Green', 'Blue'),
                          labels={1: 'tissue'}, dataset_name=task_name, license='hands off!') #0: 'background', 
